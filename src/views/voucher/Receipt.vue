<template>
  <div>
    ...
    <hr>
    <div class="text1">
      <span><i class="el-icon-star-off">&nbsp;&nbsp;收款单</i></span>
    </div>
    ...
    <el-table ref="multipleTable" border :data="dataLists" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55"></el-table-column>
      <el-table-column label="订单编号" width="200">
        <template slot-scope="scope">{{ scope.row.order_number }}</template>
      </el-table-column>
      <el-table-column prop="nickname" label="会员名" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="pattern_payment" label="支付方式" show-overflow-tooltip></el-table-column>
      <el-table-column prop="remarks" label="管理员备注" show-overflow-tooltip></el-table-column>
  ...
  export default {
    data() {
      return {
        ...
        aallSelect: true, // 列表全选与否
        multipleSelection: [],
      }
    },
    mounted:function () {
      this.receivablesList(); // 获取收款单数据列表
    },
    methods: {
      // 加载进来获取数据
      ...
      exportDatas:function(rows){
        ...
        window.location.href="http://api.zhisoufangdichan.com/api/v1/documnets/inputexcel?documents_type=1&search="+search+"&page="+page+"&limit="+limit
      },
      // 输入框，选择器，查询
      onSubmit() {
        let _this = this;
...
<style scoped>
  ...
  .el-tabs__item{ width: 65%; }
  .el-tab-pane{ width: 100%; }
</style>
