          ...
          <div slot="header" class="clearfix">
            ...
            <div class="character3">
              <el-table ref="multipleTable"
                ...
                <!--货号、属性-->
                <el-table-column label="货号/属性" width="150" show-overflow-tooltip>
                  <template slot-scope="scope">
                    <div>
                      <p>货号：{{scope.row.goods_code==null?'无':scope.row.goods_code}}</p>
                      <p>属性：{{scope.row.sku_font==null?'无':scope.row.sku_font}}</p>
                    </div>
                  </template>
                </el-table-column>
                ...
                <el-table-column prop="scope" label="库存类型" width="150" show-overflow-tooltip>
                  <template  slot-scope="scope">
                    <div>
                      <p>即时库存：{{scope.row.add_del_num==null?'无':scope.row.add_del_num}}</p>
                      <p>商品库存：{{scope.row.sku_stock==null?'无':scope.row.sku_stock}}</p>
                    </div>
                  </template>
                </el-table-column>
...
//				多选标签
    ...
    methods: {
      ...
      stockRendering(params){
        var _this=this;
        //库存的所有信息
        apiInvertory.OutOfTheTreasuryStockRes(params).then(res => {
          console.log(res)
          if(res.cscode==0){
            console.log(res)
            //总数据
