<template>
  <div class="chatList">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>客服处理</el-breadcrumb-item>
      <el-breadcrumb-item>商品客服列表</el-breadcrumb-item>
    </el-breadcrumb>
    <!--卡片查询-->
    <el-card class="card-search">
      <div slot="header" class="clearfix">
        <span>筛选查询</span>
        <el-button type="text" class="card-btn" @click="getchatlist">查询</el-button>
      </div>
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="用户昵称：">
          <el-input v-model="formInline.username" placeholder="输入用户昵称"></el-input>
        </el-form-item>
        <el-form-item label="最后聊天时间：">
          <el-date-picker v-model="formInline.up_time" type="datetime" value-format="timestamp" placeholder="选择日期时间"></el-date-picker>
        </el-form-item>
      </el-form>
    </el-card>
    <!--聊天列表-->
    <el-card class="card-list">
      ...
      <el-table
        ...
        <el-table-column prop="nickname" show-overflow-tooltip label="用户昵称" width="160"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getchatlist:function() {
        ...
        var up_times = _this.formInline.up_time;
        if(up_times != ''){
          up_times = up_times/1000;
        }
        let param = {
          types:2, // 加参数： types:2 写死
          user_id:88881, // user_id:88881 写死
          page:_this.currentPage, // 当前页码
          limit:_this.limit, // 每页多少条数据
          username:  this.formInline.username,  //用户名     【主要是搜索】
          up_time: up_times,   //最后修改时间 【主要是搜索】
          goods_name: this.formInline.goods_name, //商品名称    【主要是搜索】
        };
        console.log(param);
        param = utils.filterJson(param);
        apiChat.allChatList(param).then(res => {
          console.log(res);
          if(res.cscode === "0"){
      ...
      onSubmit() {
        // 标准格式转化为日期格式，日期格式转化为时间戳
        var up_times = (_this.formInline.up_time)/1000;
        let param = {
      ...
      handleDelete:function(id){
        this.$confirm('执行此操作将会删除本条聊天记录, 是否继续?', '删除', {
          ...
          apiChat.delChatList(param).then(res=>{
            console.log(res);
            if(res.cscode === "0"){
              this.getchatlist();
            }else{
